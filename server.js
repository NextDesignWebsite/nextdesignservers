...
const invoiceDb = mysql.createPool({
    host: process.env.DB_HOST,
    user: process.env.DB_USER,
    password: process.env.DB_PASSWORD,
...
////////////////////////// INVOICE RECOVERY APP //////////////////////////
async function invoiceRefreshToken(refresh_token) {
...
async function invoiceGetInvoices(accessToken, tenantId){
...
async function invoiceGetContact(accessToken, tenantId){
...
function invoiceGetCurrentDate() {
    const today = new Date();
...
function invoiceDaysDifference(date1, date2) { // -2 = date1 2 days BEFORE date 2
...
app.post("/invoiceapp/api/signup", requireAuth, (req, res) => {
    ...
    req.db.query("select * from users where email = ?", [email], (err, result) => {
        ...
        bcrypt.hash(password, 10, (err, hashedPassword) => {
            ...
            req.db.query(query, [name, email, hashedPassword], (err, result) => {
                if(err){
...
app.post("/invoiceapp/api/login", requireAuth, (req, res) => {
...
app.get("/invoiceapp/api/get-user", requireAuth, invoiceRequireUser, async (req, res) => {
    ...
    try {
        invoices = await invoiceGetInvoices(connections[0].access_token, tenantId);
        ...
        req.db.query("select * from notifications where user_id = ? order by id desc", [req.user.id], (err, result) => {
            ...
            req.db.query("select * from invoices where connection_id = ?", [connections[0].id], (err, result) => {
...
app.get("/invoiceapp/api/redirect-xero", (req, res) => {
...
app.post("/invoiceapp/api/create-connection", requireAuth, async (req, res) => {
...
app.get("/invoiceapp/api/checkup", async (req, res) => {
    ...
    try {
        ...
        for(const con of connections){
            ...
            try {
                xeroInvoices = await invoiceGetInvoices(con.access_token, tenantId);
            } catch(err){
                if(err.response && err.response.status == 401){
                ...
                dbInvoices.forEach(inv => {
                    if(xero.InvoiceID == inv.xero_id){
                        ...
                        contacts.forEach(contact => {
                            if(contact.ContactID == xero.Contact.ContactID) newPhone = invoiceExtractPhone(contact);
                        });
                ...
                if(xero.AmountDue != 0){
                    for(const inv of dbInvoices){
                        if(inv.xero_id == xero.InvoiceID && inv.cancelled == "no"){
                            if(invoiceDaysDifference(xeroDueDate, currentDate) <= -7 && inv.sms_stage < 1){
                                for(const contact of contacts){
                                    if(contact.ContactID == xero.Contact.ContactID){
                                        let phoneNumber = invoiceExtractPhone(contact);
                            ...
                            } else if(invoiceDaysDifference(xeroDueDate, currentDate) <= -14 && inv.sms_stage < 2){
                            } else if(invoiceDaysDifference(xeroDueDate, currentDate) <= -21 && inv.sms_stage < 3){
                            }
                        }
                    }
                }
...
app.post("/invoiceapp/api/mark-read", requireAuth, (req, res) => {
    req.db.query("update notifications set status = ? where user_id = ?", ["read", req.user.id], (err, result) => {
        if(err){
            console.error(err);
        }
...
app.post("/invoiceapp/api/reply", (req, res) => {
...
app.post("/invoiceapp/api/get-chats", requireAuth, (req, res) => {
    const xeroId = req.body.xeroId;

    req.db.query("select * from messages where xero_invoice_id = ?", [xeroId], (err, result) => {
        if(err){
            console.error(err);
        }
...
app.post("/invoiceapp/api/toggle-exclusion", (req, res) => {
    req.db.query("update invoices set cancelled = ? where id = ?", [req.body.newValue, req.body.id], (err, result) => {
        if(err){
            console.error(err);
        }
...
app.post("/job/api/send-summary", (req, res) => {
    ...
    req.db.query("select * from prices", (err, result) => {
        ...
        req.db.query("select * from jobs where id = ?", [jobId], (err, result) => {
            ...
            req.db.query("select * from users where id = ?", [jobId.user_id], (err, userResult) => {
                if(err){
                    console.error(err);
                } 
...
app.post("/job/api/new-rate", requireAuth, (req, res) => {
    const { charge, cost, id } = req.body;

    req.db.query("update users set charge = ?, cost = ? where id = ?", [Number(charge), Number(cost), id], (err, result) => {
        if(err){
            console.error(err);
        } 
...
app.post("/job/api/get-quote", (req, res) => {
...
app.post("/casey/api/send-email", async (req, res) => {
    await jobSendEmail(req.body.email, req.body.text);

    return res.json({ message: 'success' });
});
app.post("/dysons/api/dysons-email", async (req, res) => {
    await jobSendEmail("jackbaileywoods@gmail.com", req.body.text); // "info@dysonsheating.co.uk",
